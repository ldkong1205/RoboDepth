    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_split == 'cityscapes':
            ...
            dataset = datasets.KITTIRAWDataset(
                ...
                frame_idxs=frames_to_load, num_scales=4,
        ...
        if opt.eval_teacher:
            ...
            pose_enc_dict = torch.load(os.path.join(opt.load_weights_folder, "pose_encoder.pth"), map_location=device)
            pose_dec_dict = torch.load(os.path.join(opt.load_weights_folder, "pose.pth"), map_location=device)
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader):
                ...
                if opt.eval_teacher:
                    ...
                    # predict poses
                    pose_feats = {f_i: data["color", f_i, 0] for f_i in frames_to_load}  # [1, 3, 192, 640]
                    pose_feats = {k: v.to(device) for k, v in pose_feats.items()}
                    ...
                    for fi in frames_to_load:
                        if fi < 0:
                            ...
                            axisangle, translation = pose_dec(pose_inputs)
                            pose = transformation_from_parameters(
                                axisangle[:, 0], translation[:, 0], invert=False)  # [1, 4, 4])
                    ...
                    # lookup_frames = lookup_frames.to(device)
                    # relative_poses = relative_poses.to(device)
                    K = K.to(device)
                    invK = invK.to(device)
