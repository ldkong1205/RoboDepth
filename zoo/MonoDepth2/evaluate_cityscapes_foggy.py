...
splits_dir = os.path.join(os.path.dirname(__file__), "data", "cityscapes")
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        filenames = readlines(os.path.join(splits_dir, "test_filename.txt"))
        ...
        frames_to_load = [0]
        dataset = datasets.CityscapesFoggyEvalDataset(opt, opt.data_path, filenames,
        ...
        # do inference
        progress_bar = mmcv.utils.ProgressBar(len(dataloader.dataset))
        with torch.no_grad():
        ...
        log('finished predicting!')

    else:
        # Load predictions from file
        log("-> Loading predictions from {}".format(opt.ext_disp_to_eval))
    ...
    if opt.save_pred_disps:
        output_folder = os.path.join(opt.log_dir, "eval")
        if not os.path.exists(output_folder): os.makedirs(output_folder)
        output_path = os.path.join(output_folder, "cityscape.npy")
        log(f"-> Saving predicted disparities to {output_path}")
        np.save(output_path, pred_disps)

    if opt.no_eval:
        log("-> Evaluation disabled. Done.")
        quit()

    log('loading cityscapes gt depths individually due to their combined size!')
    gt_depths = os.path.join(splits_dir, "gt_depths")

    log("-> Evaluating")

    if opt.eval_stereo:
        log("   Stereo evaluation - "
              "disabling median scaling, scaling by {}".format(STEREO_SCALE_FACTOR))
        opt.disable_median_scaling = True
        opt.pred_depth_scale_factor = STEREO_SCALE_FACTOR
    else:
        log("   Mono evaluation - using median scaling")

    errors = []
    ratios = []
    progress_bar = mmcv.utils.ProgressBar(pred_disps.shape[0])
    ...
    if opt.save_pred_disps:
        ...
        output_path = os.path.join(
            opt.log_dir, "errors.npy")
        np.save(output_path, np.array(errors))

    if not opt.disable_median_scaling:
        ratios = np.array(ratios)
        med = np.median(ratios)
        log(" Scaling ratios | med: {:0.3f} | std: {:0.3f}".format(med, np.std(ratios / med)))
