    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_split == 'cityscapes':
            ...
            dataset = datasets.KITTIRAWDataset(
                ...
                on_the_fly=opt.on_the_fly,
            )
        print("On-the-fly evaludation: '{}'".format(opt.on_the_fly))
...
if __name__ == "__main__":
    ...
    #
    opt.load_weights_folder = 'models/DIFFNet/mono_640x192'
    opt.eval_mono = True
    opt.num_workers = 0
    ...
    if opt.eval_mono and not opt.eval_stereo:
        ...
        # baseline = get_scores_monodepth2_stereo()  # (MonoDepth2, stereo, 640x192)
        # logger.info("Loading baseline scores (MonoDepth2, stereo, 640x192) ...")
        # we use MonoDepth2, mono, 640x192 as the baseline for all other monocular depth estimation settings
        baseline = get_scores_monodepth2_mono()  # (MonoDepth2, mono, 640x192)
        logger.info("Loading baseline scores (MonoDepth2, mono, 640x192) ...")
