...
def create_icra(loader, save_path=None, severity_levels=[]):

    choices = np.arange(1, 19, 1, dtype=int)
    corruption = np.random.choice(choices)
    ...
    for i, batch in enumerate(loader):
        ...
        for severity in severity_levels:
            ...
            if corruption == 1:
                corrupted = corrupt(image, corruption_name='brightness', severity=severity)
                ...
                corrupted = corrupt(image, corruption_name='fog', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
            elif corruption == 4:
                corrupted = corrupt(image, corruption_name='frost', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
            elif corruption == 5:
                corrupted = corrupt(image, corruption_name='snow', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
                ...
                corrupted = corrupt(image, corruption_name='defocus_blur', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
            elif corruption == 8:
                corrupted = corrupt(image, corruption_name='glass_blur', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
            elif corruption == 9:
                corrupted = corrupt(image, corruption_name='motion_blur', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
            elif corruption == 10:
                corrupted = corrupt(image, corruption_name='zoom_blur', severity=severity)
                im_path = os.path.join(folder_path, name[0])
                if not (os.path.isdir(im_path[:-14])): os.makedirs(im_path[:-14])
                im = transforms.ToPILImage()(corrupted)
                im.save(im_path)
